...
void CD3D8Driver::createMaterialRenderers()
{
	...
	renderer = new CD3D8NormalMapRenderer(pID3DDevice, this, tmp, 
		MaterialRenderers[EMT_TRANSPARENT_VERTEX_ALPHA]);
	renderer->drop();
							 ...
							 bool vsync, bool antiAlias)
{
	...
	// set max anisotropy
	pID3DDevice->SetTextureStageState(0, D3DTSS_MAXANISOTROPY, min(16, Caps.MaxAnisotropy));
	pID3DDevice->SetTextureStageState(1, D3DTSS_MAXANISOTROPY, min(16, Caps.MaxAnisotropy));
...
{
	return new CD3D8Texture(pID3DDevice, size);
}
						...
						SColor color)
{
	...
	setRenderStates2DMode(color.getAlpha() < 255, false,
	false);
	setTexture(0,0);
...
void CD3D8Driver::draw2DRectangle(SColor color, const core::rect<s32>& position,
...
bool CD3D8Driver::setRenderStates3DMode()
{
	...
	if (Material.MaterialType >= 0 && Material.MaterialType < (s32)MaterialRenderers.size())
		shaderOK = MaterialRenderers[Material.MaterialType]->OnRender(this, LastVertexType);
...
s32 CD3D8Driver::getMaximalDynamicLightAmount()
{
...
E_DRIVER_TYPE CD3D8Driver::getDriverType()
{
	return EDT_DIRECT3D8;
}

//! Returns the transformation set by setTransform
core::matrix4 CD3D8Driver::getTransform(E_TRANSFORMATION_STATE state)
{
	return Matrices[state];
}

//! Sets a vertex shader constant.
void CD3D8Driver::setVertexShaderConstant(f32* data, s32 startRegister, s32 constantAmount)
{
	if (data)
...
void CD3D8Driver::setPixelShaderConstant(f32* data, s32 startRegister, s32 constantAmount)
{
	if (data)
